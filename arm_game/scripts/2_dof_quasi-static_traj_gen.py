...
class Generator:
    ...
    def __init__(self):
        ...
        self.kin = Kinematics(robot, 'world', 'iiwa7_link_0b') # TODO: bucket?

        # asteroid handling
        self.asteroid_handler = AsteroidHandler()
        (T_curr, J_curr) = self.kin.fkin(np.array([0.0, 0.0]).reshape([2,1]))
        self.arm_length = 2.0#np.linalg.norm(p_from_T(T_curr)) # note that arm length is calculated ignoring prismatic
        ...
        self.last_q = np.array([0.0, 0.0]).reshape([2,1])
        self.lam = 60.0
        self.lam2 = 5.0
        self.gam = 0.5
        ...
        self.qgoal = np.array([0.0, 0.0]).reshape((2, 1))
    ...
    def update_arm(self, t, dt):
        ...
        Jp = np.vstack((Jp[0:5, 0:2]))#,Jp[,0:9]))
        p = p_from_T(T)
        R = R_from_T(T)
        ...
        qdot = J_inv @ np.vstack((prdot, wrdot)) + (np.eye(2) - J_inv @ Jp) @ qdot2
        ...
        q = np.vstack((q, np.array([0.0,-0.31,1.52,0.0,1.52,-1.87,-1.52]).reshape([7,1])))
